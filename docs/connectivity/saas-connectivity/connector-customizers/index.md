...
SaaS Connectivity Customizers work by sitting in between SailPoint Human Fabric and the connector. They intercept calls from SailPoint Human Fabric to the connector and calls from the connector to SailPoint Human Fabric. When the customizer intercepts a call, it can call custom code to mutate the data in any way necessary to change the connector behavior.
