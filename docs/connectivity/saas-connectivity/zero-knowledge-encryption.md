...
Many enterprises take advantage of SailPoint Human Fabric (SHF) to govern access, enhance productivity, and automate IT processes across both cloud and on-premises resources. 
...
SHF secures all credentials and sensitive identity data while at rest and in motion. SailPoint’s solution for securing sensitive data is built upon two tenets:
...
SHF delivers this level of security using a scalable and redundant architecture that, unlike many other solutions in the market, does not rely on using agents or opening the corporate firewall, and never exposes sensitive data to attack.
...
The VA continuously polls SHF in the cloud, looking for tasks or for data that is waiting for action. 
...
The VA then serves the public key that SHF uses to encrypt account credentials. 
...
Whenever the user signs in to SHF using their password on subsequent logins, this one-way hash is generated anew, authenticating the user into SHF based on the hash value.

#### Pass-Through authentication

Pass-through authentication allows SHF users to authenticate using a trusted source of authentication, such as the company’s Active Directory server. 
When a pass-through authentication user creates or changes their password, SHF uses *Zero Knowledge Encryption* to encrypt the new password in the browser using the 2048-bit RSA public key hosted by the virtual appliance. 
...
When the user signs in to SHF in the future, this 2048-bit encrypted representation of the password is sent to the VA where the target system password is derived, within the customer’s firewall. 
...
In SHF, the answers to users’ security questions are encrypted using SHA-256 to create a one-way hash, using a salt that includes a random string and the user’s Universally Unique Identifier (UUID). 
...
Because SHF enables many access-oriented actions for signed-in users, it becomes critical to understand how those users can be signed in to SHF.

When a user is signing in to SHF from a third-party SSO tool, the security of the credentials used for this sign-in depends on the protocols or methods that SSO tool uses. 
...
To ensure that credentials cannot be compromised during credential replay, it's critical that you thoroughly evaluate how your SSO provider stores credentials and secures them when it's replaying into SHF. 
Because these methods vary and don't conform to a recognized standard, credential replay for signing in to SHF is not the preferred method of single sign-on.
...
SHF fully supports SAML for federated logins, which enables integration with the best in class access management products without putting critical credentials at risk.

### Secure password management

When the user updates a password on a target application through SHF, they must use the administrative credentials for that target system or directory to set the new password. 
SHF uses *Zero Knowledge Encryption* through the SailPoint VA to secure these sensitive administrative credentials. 
...
SHF implements *Zero Knowledge Encryption* within the SailPoint virtual appliance to secure all these administrative actions that are used by automated tasks within the governance platform:

- **Account Aggregation** – SHF uses administrative credentials to learn about accounts on diverse types of connected systems.
- **User Provisioning** – SHF uses administrative account access to external systems to provision accounts, set entitlements, lock and unlock or enable and disable accounts, assign groups, and perform other actions on those systems.
- **Password Management** – Changing and updating passwords on external systems requires SHF to sign in to those systems with administrative access.
...
All these automated tasks, and any other administrative action SHF takes as a part of the service of governance, are all secured using *Zero Knowledge Encryption* through the VA.
...
SailPoint Human Fabric builds upon decades of enterprise security best practices, and unlike other vendors, does not require customers to change their firewall or intrusion protection practices. 
