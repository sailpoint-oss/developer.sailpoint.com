...
**Backup** and **deploy** in Configuration Hub are the same operations as **export** and **import** with the SaaS Configuration APIs — Configuration Hub uses these APIs behind the scenes. A backup is an export of a tenant's configuration, and a deploy imports objects from a backup into a target tenant.
...
:::caution
A ✅ in the **Import** or **Deploy** column indicates support for standard sources only. Non-Employee sources are **not** supported for import or deploy, even when deploying from a backup of another tenant. See the [note below](#importing-configurations) for details.
:::

:::caution
A ❌ in the **Import** column means the SaaS Configuration import API cannot import that object type. Including one in an import payload does not fail the job — the unsupported object is skipped and the import still completes. See [Importing configurations](#importing-configurations) for details.
:::
...
This limitation applies to both terms used for this functionality:

- You cannot **import** a Non-Employee source with the SaaS Configuration APIs.
- You cannot **deploy** a Non-Employee source from a backup in Configuration Hub, including a backup taken from a different tenant (for example, deploying a sandbox backup into production).

If you attempt to deploy a Non-Employee source from a backup, the source is created as a standard source rather than a Non-Employee source. This can cause issues in the target tenant. Because this behavior is expected, the **Import** and **Deploy** columns in the [Supported objects](#supported-objects) table reflect standard source support only and do not apply to Non-Employee sources.

:::

:::caution Unsupported object types are skipped without an error

Importing is not all-or-nothing. If an import payload contains a mix of supported and unsupported object types, the import job imports the supported types, bypasses the unsupported types, and still finishes with a `status` of `COMPLETE`. The job is not interrupted partway through, and no error or warning tells you which objects were skipped.

Before you build an import payload, check the **Import** column in the [Supported objects](#supported-objects) table. A ❌ means the import API cannot import that object type, even when the same object type shows a ✅ in the **Deploy** column.

To confirm what an import actually changes, do either of the following:

- Run the import with `preview=true` first, then review the result. Refer to [Import objects](#import-objects).
- Compare your payload against the [import results](#import-results), which list only the objects that were imported.
...
[This endpoint](https://developer.sailpoint.com/docs/api/list-sp-config-objects-v-1) gets the list of object configurations the tenant export/import service knows. 
...
[This endpoint](https://developer.sailpoint.com/docs/api/export-sp-config-v-1) exports selected objects from the tenant to a JSON configuration file.
...
[This endpoint](https://developer.sailpoint.com/docs/api/get-sp-config-export-status-v-1) gets the status of the export job identified by its `id` parameter.
...
[This endpoint](https://developer.sailpoint.com/docs/api/get-sp-config-export-v-1) gets the export job's resulting export file by using the requested `id` and downloads it to a file.
...
[This endpoint](https://developer.sailpoint.com/docs/api/import-sp-config-v-1) imports objects from a JSON configuration file into a tenant. 
...
[This endpoint](https://developer.sailpoint.com/docs/api/get-sp-config-import-status-v-1) gets the status of the import job identified by its `id` parameter. 
...
[This endpoint](https://developer.sailpoint.com/docs/api/get-sp-config-import-v-1) gets the import job's resulting import file by using the requested `id` and downloads the file. 
