...
- Based on the ID received, an approval task is assigned to the identity or governance group in SailPoint Human Fabric for a decision as an additional step after other configured approval requirements are met.
...
You can use this trigger to develop logic outside of SailPoint Human Fabric’s out-of-the-box offerings to route an approval step to users such as the following:
