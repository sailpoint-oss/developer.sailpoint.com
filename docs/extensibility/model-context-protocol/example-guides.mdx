...
Some identities may have multiple accounts configured for a single identity. If so, the output of the `list-requestable` tool
will tell us that account selection is required. We will need the results of the `list-requestable` tool output in order
to use the `create-access-request` tool and make the appropriate account selections.
...
In this response, we can see `"accountSelectionRequired": true` and now we have an `account` field which is populated
with a source `Docusign SaaS` and a list of accounts available on the source `accountsOnSource`. We will need to provide
one of those accounts when requesting access. The account provided is the account that will be granted access when the
request is processed/approved.
...
from whichever account we have selected. Names are provided in the `list-requestable` output for readability, but only
...
tool can be called like in the basic example above.
...
```
Here in `accounts` we provided two items, one for each source found in the `list-requestable` output. Each item contains
the source ID and that native identifier of the account we have selected. The accounts provided are the accounts that will
get access when the request is processed/approved.

## Advanced: access request, account selection required, multiple sources, 0 to many accounts
The most complicated example is where we are requesting access to a `ROLE` which has many sources and those sources have
different account selection needs. For example, the role has three sources. In source 1, the identity has exactly one account.
In source 2, the identity has 2 (or more) accounts. In source 3, the identity has no accounts yet. (No accounts will happen
when the identity is requesting access to something that they have never accessed before and don't already have an account provisioned.)
...
The identity has no account created yet on `multi_source_1`. The user has exactly one account on `multi_source_2`. The
