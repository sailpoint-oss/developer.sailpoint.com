...
1. Use the [Search endpoint](https://developer.sailpoint.com/docs/api/search-post-v-1) to identify the access profiles that are **not** associated with applications configured for access requests. Sending the following query will return a list of these unassociated access profiles.
...
2. Use the [PATCH Access Profile endpoint](https://developer.sailpoint.com/docs/api/patch-access-profile-v-1) and provide the unassociated access profile's `id` to update the specified access profile's `requestable` field.
...
2. Use the [Update Requestability for Access Profiles endpoint](https://developer.sailpoint.com/docs/api/update-access-profiles-in-bulk-v-1) and provide every unassociated access profile's `id`, along with the updated values for their `requestable` fields.
