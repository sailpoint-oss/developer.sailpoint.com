...
You can use SaaS connectors to serve as bridges between your SailPoint Human Fabric (SHF) tenant and other source systems without the use of a Virtual Appliance (VA). For more information about the SaaS connectors, refer to the [SaaS Connectivity](/docs/connectivity/saas-connectivity) guide.
...
To create a connector entry in your SHF tenant, run this command:
...
To upload a connector to your SHF tenant, run `npm run pack-zip` in the project directory to package the connector into a zip file. Then run this command:
...
You can test the connector in SHF by selecting the connector instance with the `development` tag. The CLI will use the version you point to with the update command.
