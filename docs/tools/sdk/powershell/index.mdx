...
description: Easy SHF development in PowerShell. 
...
- Your tenant name in IdentityNow. To learn how to find it, refer to [Getting Started](/docs/api/getting-started#find-your-tenant-name). The SDK will use this tenant name to connect to your SailPoint Human Fabric instance.
...
The command installs the SailPoint PowerShell SDK module. Starting with version 2.0, the SDK is a single, flat module (`PSSailpoint`) organized by API resource - there are no longer separate `PSSailpoint.Beta`, `PSSailpoint.V3`, or `PSSailpoint.V2024` modules to install. You will be prompted to confirm that you want to install the module from 'PSGallery'. Enter "A" to say "Yes to All".
...
To validate that the module is installed, run this command, `Get-Module -ListAvailable PSSailpoint`, and verify that the module is listed. Additionally, you can run this command, `Get-SailPointCommand`, to see the available commands included in the SDK.
...
5. To validate that the module is installed, run `Get-Module -ListAvailable PSSailpoint` and verify that the module is listed. Additionally, you can run `Get-SailPointCommand` to see the SDK's available commands.
