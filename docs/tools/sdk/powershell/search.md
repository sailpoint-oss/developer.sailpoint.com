...
The PowerShell SDK provides you access to SailPoint Human Fabric's [Search](https://documentation.sailpoint.com/saas/help/search/index.html) functionality.
...
try {
    ...
    Write-Host ("Exception occurred when calling Search-PostV1: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```

This example request uses the [Perform Search endpoint](/docs/api/search-post-v-1) to search your tenant for identities with the name "john.doe".
...
This example searches your SailPoint Human Fabric tenant for all identities and sorts them by their `displayName` in descending order. The search returns a maximum of 150000 records (the `Limit`) and 5000 records per page (the `Increment`).
