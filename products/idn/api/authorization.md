...
In order to maintain the least level of privilege for users, it is advisable to assign each user the lowest [user level](https://documentation.sailpoint.com/saas/help/common/users/user_level_matrix.html) possible for them to do their job and then attempt to add the required scopes from there.  If the user requires certain scopes that fall outside of their current user level, then consider raising the user level to accommodate those scopes.  This approach ensures the user has the least amount of privilege in both the UI and the API.
