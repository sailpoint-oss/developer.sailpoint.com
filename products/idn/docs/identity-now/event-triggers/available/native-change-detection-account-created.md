...
Native Change Detection events occur when a source account is changed directly on the source as opposed to changing as the result of a provisioning action initiated by SailPoint.  This trigger fires an event when an account is **created** directly on the target source rather than from a provisioning activity by SailPoint.
