...
Unlike Cloud Rules, Connector Rules do not have a rule review process and are directly editable via [Connector Rule REST APIs](https://developer.sailpoint.com/apis/beta/#tag/Connector-Rule-Management). For more details, see [Configuration Process](#configuration-process).
...
| Build Map Rule       | BuildMap              | Delimited File                           | Calculates and transforms data from a parsed file during the aggregation process. *Note: This is only available for the Delimited File source type, and not Generic source types.* |
| JDBC Build Map Rule  | JDBCBuildMap          | JDBC                                     | Calculates and transforms data from a database query result during the aggregation process. Additionally, can perform additional calls back to the database. *Note: This rule is available for the JDBC generic source as well as for other sources which derive from the JDBC connector (e.g., Oracle EBS, PeopleSoft, etc.)* |
...
Connector Rules are directly editable via [Connector Rule REST APIs](https://developer.sailpoint.com/apis/beta/#tag/Connector-Rule-Management), which provide ability to interact with rules directly.
...
SailPoint architectural optimizations have added resiliency and protections against malformed or long-running rules. In addition, these APIs also offer built-in protection and checking against potentially harmful code - see [Rule Code Restrictions](../rule_guide.md#rule-code-restrictions).
...
- `id` - the unique UUID that the REST APIs refers to this rule by. This is generated on creation.
- `name` - the name that the user interface and references might refer to this rule by.
...
- `type` - the type of connector rule this is. For a list of supported rule types, see [Supported Connector Rules](#supported-connector-rules).
- `attributes` - a list of attributes
  - `sourceVersion` - A string which indicates the version of the rule. Typically the same as `version`
- `sourceCode` - this object houses the actual source code which makes the rule work
  - `version` - A string which indicates the version of the rule. Typically the same as `sourceVersion`.
  - `script` - The rule’s code that is run by the connector.

## Attaching Connector-Related Rules to Sources

Once a connector-related rule has been imported to your tenant, you will need to configure any sources which need to reference that rule during the desired operation. This configuration is accomplished through the execution of an API call on the source. The below examples all use a `PATCH` operation for a partial source update, however `PUT` operations will work, too, as long as the entire source object model is provided.

For the `PATCH` operations, an `op` key has to be provided. For new configurations this is typically set to `add` as our example shows, however they can be any of the following:
...
- `remove` - Removes a value from the configuration. Use this operation if you want to unset a value. **Beware: Removals can be destructive if the path is not configured properly. This could negatively alter your source config.**
...
*Note: Replace `*` with the index location of operation as it is configured on the source. For example 0, 1, 2, etc. You can do a `GET` call on the source first to verify the index location prior to executing the `PATCH` call to attach the rule.*
