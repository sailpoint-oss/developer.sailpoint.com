...
In this mini hack, you will turn an identity event into a message. You will finish a partly-built Workflow Studio
...
You are building one step. That sounds small, but it is the step where two systems that know nothing
...
**End** step. Execution runs straight down — no branching here, though branching is possible in a more advanced flow.

Two things to note about the shape of it. Every step **points at the next one**, and the last step points at
...
And your work goes **between `Get Manager` and the End**. Everything above that is a working example
of one pattern.
...
This is worth understanding, because it explains a lot of what you will see later:
...
Two consequences are worth noticing now. A step can only see steps *above* it, which is why order
...
There are already runs in here, and they are not yours, yet. Setup loaded the eight baseline people, which
...
as a demonstration of the problem you can use should you decide to try fixing the flow.
