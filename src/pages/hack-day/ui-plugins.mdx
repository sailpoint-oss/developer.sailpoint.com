...
:::tip Here for the main hack?

Start with this track. It teaches you how UI plugins work, and it takes about one hour. When you
finish, go to [Take it to the main hack](#take-it-to-the-main-hack) and use what you learned to
...
![The finished manager lookup page](screenshots/ui-plugins-finished.png)
...
Download `sail_2.4.1_windows_amd64.msi` from the
...
```bash
sudo apt install ./sail_2.4.1_linux_amd64.deb
```

**Your terminal — Red Hat or Fedora**

```bash
sudo yum localinstall ./sail_2.4.1_linux_amd64.rpm
```

  </TabItem>
</Tabs>

Confirm that the version is **2.4.1 or newer**:
...
[UI Plugins: an in-depth look](#ui-plugins-an-in-depth-look) for more about the theme.
...
<details>
<summary>See details: why not localhost, other ports, and how to unlink</summary>
...
![Checkpoint 1 page](screenshots/ui-plugins-checkpoint-1.png)
...
`plugin.status()`, and `plugin.apiReady()`. See [UI Plugins: an in-depth look](#ui-plugins-an-in-depth-look).
...
2. Above `private requested = false`, add the list of managers. Then **uncomment the
...
![Checkpoint 2 page](screenshots/ui-plugins-checkpoint-2.png)
...
1. Below the `managers` method, add the list of reports. Then **uncomment the `return` line and delete the
...
2. Below the `onManagerChange()` method, add three helpers for the table. They read the department, make
...
3. In `src/app/app.html`, add the table after the `picker` section, inside `<main>`:
...
4. Save, refresh, and pick a manager.
...
![Checkpoint 3 page](screenshots/ui-plugins-finished.png)
...
- `attr()` reads a value from `attributes`. Department and type are not top-level fields — they live in
  `attributes`, which the SDK types as a plain `object` because its contents differ in each tenant.
- `initials()` turns a name into initials for the avatar.
- `typeSeverity()` maps an identity type to a PrimeNG severity: `EMPLOYEE` is green (`success`) and
  `CONTRACTOR` is yellow (`warn`), so the colour of the type tag carries meaning at a glance.
...
You now know what a UI plugin can do. It runs inside ISC, it uses the signed-in user's session, and it
can call SailPoint APIs with no extra authentication. **Now, what do you want to build?**

You can submit a UI plugin as your main hack. The judges score it with the same
[judging criteria](/hack-day/judging-criteria) as the MCP Server hack, and it can win the same
prizes.

Start with a real problem. Think of a task that takes too many clicks in ISC today, or a question
that a manager, an auditor, or a help desk agent cannot answer easily. Then build the page that
solves it.

:::note Before you submit

Your plugin must meet the qualifying requirements in the
[judging criteria](/hack-day/judging-criteria#qualifying-requirement-passfail--not-scored). It must
use at least one SailPoint API, and you must be able to show it live in your tenant.
