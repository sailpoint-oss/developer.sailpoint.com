...
import { sendCode, SailAppsAuthError } from '../services/SailAppsService';
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result?.message === 'Token added successfully') {
        ...
        setPageStatus('error');
        setResultDisplay(
          getApiErrorDisplay('Authentication failed: Unexpected response from server'),
        );
      }
    } catch (error) {
      setPageStatus('error');
      ...
      <div className={styles.gettingStartedText}>
        ...
        {pageStatus === 'ready' ? (
          <p className={styles.gettingStartedTwo}>
            ...
            confirmation code below before granting access.
          </p>
        ) : null}
