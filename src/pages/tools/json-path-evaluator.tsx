...
const JsonPathEvaluator: React.FC = () => {
  ...
  const applyJsonPathQuery = useCallback(
    async (json: string, jsonPath: string) => {
      ...
      try {
        ...
        try {
          const apiEndpoint = siteConfig.customFields
            ?.CMS_APP_API_ENDPOINT as string;
  ...
  const handleImplementationChange = useCallback((value: string) => {
    // cast to your union type
    setImplementation(value as ImplementationType);
  }, []);
