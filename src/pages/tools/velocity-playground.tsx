...
const SAMPLE_TEMPLATE = `#macro(renderProvisioningPhaseIST $phases)
  #foreach($phase in $phases)
       ...
       Access Request Phase Name:$phase.name
        #if($phase.started)
          Access Request Started (IST): $date.format("yyyy-MM-dd HH:mm:ss", $date.toDate($phase.started), "Asia/Kolkata")
        #else
          Access Request Started:Not Available
        #end
        #if($phase.finished)
          Access Request Finished (IST): $date.format("yyyy-MM-dd HH:mm:ss", $date.toDate($phase.finished), "Asia/Kolkata")
        #else
...
export default function VelocityPlayground() {
  ...
  const onKeyRun = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
  ...
  return (
    <Layout title="Velocity PlayGround" description="The SailPoint Developer Community has everything you need to build, extend, and automate scalable identity solutions.">
   <main>
      ...
      <main className={styles.grid}>
        ...
        <section className={styles.pane}>
          ...
          <textarea
            ...
            spellCheck={false}
            onKeyDown={onKeyRun}
            aria-label="Velocity template input"
          />
        ...
        <section className={styles.pane}>
          ...
          <pre className={styles.output} aria-live="polite">
            {state.error ? state.error : state.output}
          </pre>
        </section>
      </main>
    </div>
